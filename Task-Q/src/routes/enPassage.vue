  ...
  <div class="Passage">
    <div class="alls">
      <div class="head">
        ...
        <p class="headWords">Youth League Committee </p>
        <p class="headWords" v-show="$route.path=='/en/Catalogs'||$route.path=='/en/search'">>{{cat}}</p>
        <p class="headWords" v-show="$route.path=='/en/Passage'">>{{pas}}</p>
      </div>
      <div class="passage" v-if="$route.path=='/en/Passage'">
        <div class="hcontents">
          <h2 class="articleTitle">{{firsthead}}</h2>
          <h3 class="articleSub">{{secondehead}}</h3>
          <p class="otherInformation">Sourse：{{source}}   Publish：{{publish}}  Views：{{views}}</p>
          <HR align=center width=80% color=black size=1></HR>
          <div class="content" v-html="content"></div>
          <p class="author" >{{author}}</p>
        </div>
        <ul class="attachments">
          <!-- <div class="attachmentDownload" v-show="attachments.length>0">Attachments</div> -->
          <li v-for="(attachment,i) in attachments" class="attachment">
            <div @click="getAttachmentLink(attachment.attachment_path)" class="atN">Attachments{{i+1}} </div>
            <p class="downloadNumber">:DownloadNumber{{attachment.download_number}}次</p>
          </li>
        </ul>
      </div>
      <div class="catalogs" v-show="$route.path=='/en/Catalogs'||$route.path=='/en/search'">
        <div v-show="$route.path=='/en/search'" >
          <div class="catalogGuide">{{cat}}Catalogs</div>
        ...
        <div v-show="$route.query.id==20&&$route.path!='/en/search'" class="services">
        ...
        <el-pagination v-show="$route.query.id!=20||$route.path=='/en/search'" background layout="prev, pager, next" :total="$root.total" :page-size="20"
...
export default{
  ...
  methods:{
    handleCurrentChange(val){
      if(this.$route.path=='/en/Catalogs'){
        this.$root.catalogs=[]
        this.$router.push({path:'/en/Catalogs',query:{id:this.$route.query.id,page:val}})
        ...
        this.$root.catalogs=[]
        this.$router.push({path:'/en/search',query:{key:this.$route.query.key,page:val}})
    ...
    getAttachmentLink(l){
      axios.get('api/v1/eAttachment',{params:{id:l}})
