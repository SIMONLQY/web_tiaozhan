<template>
  <div class="all">
    ...
    <div class="concentrete">
      <div class="tr">
        <div class="notice" @mouseover="showm" v-bind:class="{isActive: underline}">通知公告</div>
        <div class="notice" @mouseover="showf" v-bind:class="{isActive:!underline}">媒体聚焦</div>
      </div>
      <div class="focuses">
        ...
        <div  v-for="(focuse,j) in focuses" v-show="fisActive" class="focuse" >
          <div class="focuseIp">
            <img src="../assets/guideShape.png" class="guideShape">
            <div class="noticeCatalogHead" @click="getNoticeCatalog(focuse.article_id)">{{focuse.headline}}</div>
          </div>
          <div class="noticeCatalog" @click="getNoticeCatalog(focuse.article_id)">{{focuse.publish}}</div>
        </div>
      </div>
      <a  class="ForMore" @click="forMore">查看更多</a>
    </div>
  </div>
</template>
...
<script type="text/javascript">
  ...
  export default{
    ...
    methods:{
      ...
      forMore(){
        if(this.misActive==true){
          this.$router.push({path:'/Catalogs',query:{id:9,page:1}})
    ...
    created(){
      axios.get('/api/v1/headlineNewsColumn')
      .then(r=>this.checktopline(r.data))
      axios.get('/api/v1/noticeColumn',{params:{id:9}})
      .then(r=>this.getCatts(r.data))
      axios.get('/api/v1/noticeColumn',{params:{id:10}})
      .then(r=>this.getFocuses(r.data))
    }
  }
;</script>
